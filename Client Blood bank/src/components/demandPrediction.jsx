...
function DemandPrediction() {
  ...
  const getDataSummed = (data) => {
    ...
    Object.keys(predictions).forEach((key) => {
      ...
      if (
        updatedPredictionData[bloodGroup] &&
        updatedPredictionData[bloodGroup][rhFactor]
      ) {
  ...
  const handleLoadData = () => {
    ...
    axios
      .post("http://127.0.0.1:4600/predict", mlBody)
      .then((res) => {
  ...
  const renderTableRows = () => {
    return Object.keys(predictionData).map((bloodType) => (
      <React.Fragment key={bloodType}>
        <tr>
          <td rowSpan={6} className="border-2 text-center">
            {bloodType}
          </td>
          <td rowSpan={3} className="border-2 text-center">
            Positive
          </td>
          <td className="border-2 text-center">RBC</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Positive.RBC}
          </td>
        </tr>
        <tr>
          <td className="border-2 text-center">Plasma</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Positive.Plasma}
          </td>
        </tr>
        <tr>
          <td className="border-2 text-center">Platelets</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Positive.Platelets}
          </td>
        </tr>
        <tr>
          <td rowSpan={3} className="border-2 text-center">
          ...
          <td className="border-2 text-center">RBC</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Negative.RBC}
          </td>
        </tr>
        <tr>
          <td className="border-2 text-center">Plasma</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Negative.Plasma}
          </td>
        </tr>
        <tr>
          <td className="border-2 text-center">Platelets</td>
          <td className="border-2 text-center">
            {predictionData[bloodType].Negative.Platelets}
          </td>
        </tr>
      </React.Fragment>
    ));
        ...
        <div className="grid grid-cols-3 pb-5">
          ...
          <div className="col-span-1 flex space-x-10">
            <button
              ...
              className="flex space-x-3 text-white bg-bloodred3 items-center px-5 p-1 rounded-full"
            >
              <FeatherIcon icon="settings" /> <span> Advanced options</span>
            </button>
            <button
              disabled={predictionData.A.Positive.RBC == ""}
              onClick={exportCSVFile}
              className={`${
                predictionData.A.Positive.RBC == "" ? "bg-bloodred3/40" : ""
              } flex space-x-3 text-white bg-bloodred3 items-center px-5 p-1 rounded-full`}
            >
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="">
                ...
                <select
                  ...
                  id="blood_group"
                  onChange={(e) =>
                    setMlBody({ ...mlBody, blood_group: e.target.value })
                  }
                >
              ...
              <div className="">
                ...
                <select
                  ...
                  id="blood_product"
                  onChange={(e) =>
                    setMlBody({ ...mlBody, blood_product: e.target.value })
                  }
                >
        ...
        {predictionData.A.Positive.RBC != "" ? (
          <div className="py-5 h-full">
            <table className="w-full border-2">
              <caption className="text-2xl text-bloodred1">
                {tableTitle}
              </caption>
              <thead>
                <tr>
          ...
          <h1 className="text-bloodred1/10 text-4xl flex items-center justify-center">No Predictions to Display</h1>
        </div>)
      }
      </div>
    </>
